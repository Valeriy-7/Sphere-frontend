...
export const completeRegistrationDtoSchema = z.object({
  ...
  apiKey: z.string().min(32).describe('API ключ Wildberries (обязательное поле для продавцов Wildberries)').optional(),
  inn: z
    .string()
    .regex(/^\d{10}$|^\d{12}$/)
    .describe('ИНН компании (обязательное поле для фулфилмента)')
    .optional(),
})