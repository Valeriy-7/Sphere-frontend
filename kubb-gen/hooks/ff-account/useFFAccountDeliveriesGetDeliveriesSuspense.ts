...
export const FFAccountDeliveriesGetDeliveriesSuspenseQueryKey = (params?: FFAccountDeliveriesGetDeliveriesQueryParamsType) =>
  [{ url: '/ff-account/deliveries' }, ...(params ? [params] : [])] as const

export type FFAccountDeliveriesGetDeliveriesSuspenseQueryKey = ReturnType<typeof FFAccountDeliveriesGetDeliveriesSuspenseQueryKey>
...
>(
  params?: FFAccountDeliveriesGetDeliveriesQueryParamsType,
  options: {
    query?: Partial<UseSuspenseQueryOptions<FFAccountDeliveriesGetDeliveriesQueryResponseType, ResponseErrorConfig<Error>, TData, TQueryKey>>
    client?: Partial<RequestConfig> & { client?: typeof client }
  } = {},
) {
  const { query: queryOptions, client: config = {} } = options ?? {}
  const queryKey = queryOptions?.queryKey ?? FFAccountDeliveriesGetDeliveriesSuspenseQueryKey(params)

  const query = useSuspenseQuery({
    ...(FFAccountDeliveriesGetDeliveriesSuspenseQueryOptions(params, config) as unknown as UseSuspenseQueryOptions),
    queryKey,
