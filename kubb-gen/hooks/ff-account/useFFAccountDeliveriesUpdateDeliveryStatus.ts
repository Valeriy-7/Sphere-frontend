...
export const FFAccountDeliveriesUpdateDeliveryStatusMutationKey = () => [{ url: '/ff-account/deliveries/{id}/status' }] as const

export type FFAccountDeliveriesUpdateDeliveryStatusMutationKey = ReturnType<typeof FFAccountDeliveriesUpdateDeliveryStatusMutationKey>
...
export async function FFAccountDeliveriesUpdateDeliveryStatus(
  ...
  config: Partial<RequestConfig<FFAccountDeliveriesUpdateDeliveryStatusMutationRequestType>> & { client?: typeof client } = {},
) {
  const { client: request = client, ...requestConfig } = config

  const res = await request<
    FFAccountDeliveriesUpdateDeliveryStatusMutationResponseType,
    ResponseErrorConfig<FFAccountDeliveriesUpdateDeliveryStatus400Type | FFAccountDeliveriesUpdateDeliveryStatus404Type>,
    FFAccountDeliveriesUpdateDeliveryStatusMutationRequestType
...
export function useFFAccountDeliveriesUpdateDeliveryStatus<TContext>(
  options: {
    mutation?: UseMutationOptions<
      FFAccountDeliveriesUpdateDeliveryStatusMutationResponseType,
      ResponseErrorConfig<FFAccountDeliveriesUpdateDeliveryStatus400Type | FFAccountDeliveriesUpdateDeliveryStatus404Type>,
      { id: FFAccountDeliveriesUpdateDeliveryStatusPathParamsType['id']; data: FFAccountDeliveriesUpdateDeliveryStatusMutationRequestType },
      TContext
    >
    client?: Partial<RequestConfig<FFAccountDeliveriesUpdateDeliveryStatusMutationRequestType>> & { client?: typeof client }
  } = {},
) {
  const { mutation: mutationOptions, client: config = {} } = options ?? {}
  const mutationKey = mutationOptions?.mutationKey ?? FFAccountDeliveriesUpdateDeliveryStatusMutationKey()
