...
export type AvatarUploadAvatarByInnMutationKey = ReturnType<
  typeof avatarUploadAvatarByInnMutationKey
>;

/**
 * @description     Загружает файл аватарки для кабинета по ИНН компании.    ### Процесс загрузки:    1. Поиск кабинета по ИНН    2. Валидация файла (размер, формат)    3. Оптимизация изображения    4. Загрузка в хранилище    5. Обновление URL в базе данных    ### Требования:    - Максимальный размер: 5MB    - Форматы: JPG, PNG, WebP    - Минимальное разрешение: 200x200
...
export async function avatarUploadAvatarByInn(
  ...
  config: Partial<RequestConfig<AvatarUploadAvatarByInnMutationRequestType>> & {
...
export function useAvatarUploadAvatarByInn<TContext>(
  options: {
    mutation?: UseMutationOptions<
      ...
      TContext
    >;
    client?: Partial<RequestConfig<AvatarUploadAvatarByInnMutationRequestType>> & {
      client?: typeof client;
    };
  } = {},
) {
