...
/**
 * @description     Загружает файл аватарки для кабинета.    ### Процесс загрузки:    1. Валидация файла (размер, формат)    2. Оптимизация изображения    3. Загрузка в хранилище    4. Обновление URL в базе данных    ### Требования:    - Максимальный размер: 5MB    - Форматы: JPG, PNG, WebP    - Минимальное разрешение: 200x200    - Максимальное разрешение: 1200x1200
...
export async function avatarUploadAvatar(
  ...
  config: Partial<RequestConfig<AvatarUploadAvatarMutationRequestType>> & { client?: typeof client } = {},
) {
  ...
  const res = await request<AvatarUploadAvatarMutationResponseType, ResponseErrorConfig<Error>, AvatarUploadAvatarMutationRequestType>({
    method: 'POST',
...
export function useAvatarUploadAvatar<TContext>(
  options: {
    mutation?: UseMutationOptions<
      ...
      { cabinetId: AvatarUploadAvatarPathParamsType['cabinetId']; data?: AvatarUploadAvatarMutationRequestType },
      TContext
    >
    client?: Partial<RequestConfig<AvatarUploadAvatarMutationRequestType>> & { client?: typeof client }
  } = {},
) {
