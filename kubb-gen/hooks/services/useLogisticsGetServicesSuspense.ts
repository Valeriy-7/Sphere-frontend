...
export type LogisticsGetServicesSuspenseQueryKey = ReturnType<
  typeof logisticsGetServicesSuspenseQueryKey
>;
 ...
 */
export async function logisticsGetServicesSuspense(
  config: Partial<RequestConfig> & { client?: typeof client } = {},
) {
  const { client: request = client, ...requestConfig } = config;

  const res = await request<
    LogisticsGetServicesQueryResponseType,
...
export function logisticsGetServicesSuspenseQueryOptions(
  config: Partial<RequestConfig> & { client?: typeof client } = {},
) {
