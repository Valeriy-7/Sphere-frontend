...
export type UsersGetCurrentUserSuspenseQueryKey = ReturnType<
  typeof usersGetCurrentUserSuspenseQueryKey
>;
 ...
 * {@link /users/me}
 */
export async function usersGetCurrentUserSuspense(
  config: Partial<RequestConfig> & { client?: typeof client } = {},
) {
  const { client: request = client, ...requestConfig } = config;

  const res = await request<
    UsersGetCurrentUserQueryResponseType,
    ResponseErrorConfig<Error>,
    unknown
  >({ method: 'GET', url: `/users/me`, ...requestConfig });
  return res.data;
}

export function usersGetCurrentUserSuspenseQueryOptions(
  config: Partial<RequestConfig> & { client?: typeof client } = {},
) {
