 ...
 * {@link /cabinets}
 */
export async function cabinetsFindAllSuspense(
  config: Partial<RequestConfig> & { client?: typeof client } = {},
) {
  const { client: request = client, ...requestConfig } = config;

  const res = await request<CabinetsFindAllQueryResponseType, ResponseErrorConfig<Error>, unknown>({
    method: 'GET',
    url: `/cabinets`,
    ...requestConfig,
  });
...
export function cabinetsFindAllSuspenseQueryOptions(
  config: Partial<RequestConfig> & { client?: typeof client } = {},
) {
  const queryKey = cabinetsFindAllSuspenseQueryKey();
  return queryOptions<
    CabinetsFindAllQueryResponseType,
