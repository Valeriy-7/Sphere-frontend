...
export async function cabinetsSetActive(
  ...
  config: Partial<RequestConfig<CabinetsSetActiveMutationRequestType>> & { client?: typeof client } = {},
) {
  const { client: request = client, ...requestConfig } = config

  const res = await request<CabinetsSetActiveMutationResponseType, ResponseErrorConfig<CabinetsSetActive404Type>, CabinetsSetActiveMutationRequestType>({
    method: 'PUT',
...
export function useCabinetsSetActive<TContext>(
  options: {
    mutation?: UseMutationOptions<
      ...
      TContext
    >
    client?: Partial<RequestConfig<CabinetsSetActiveMutationRequestType>> & { client?: typeof client }
  } = {},
) {
