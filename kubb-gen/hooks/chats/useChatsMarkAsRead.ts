...
export async function chatsMarkAsRead(
  ...
  config: Partial<RequestConfig<ChatsMarkAsReadMutationRequestType>> & { client?: typeof client } = {},
) {
  const { client: request = client, ...requestConfig } = config

  const res = await request<ChatsMarkAsReadMutationResponseType, ResponseErrorConfig<Error>, ChatsMarkAsReadMutationRequestType>({
    method: 'POST',
...
export function useChatsMarkAsRead<TContext>(
  options: {
    mutation?: UseMutationOptions<ChatsMarkAsReadMutationResponseType, ResponseErrorConfig<Error>, { data: ChatsMarkAsReadMutationRequestType }, TContext>
    client?: Partial<RequestConfig<ChatsMarkAsReadMutationRequestType>> & { client?: typeof client }
  } = {},
) {
  const { mutation: mutationOptions, client: config = {} } = options ?? {}
  const mutationKey = mutationOptions?.mutationKey ?? chatsMarkAsReadMutationKey()

  return useMutation<ChatsMarkAsReadMutationResponseType, ResponseErrorConfig<Error>, { data: ChatsMarkAsReadMutationRequestType }, TContext>({
