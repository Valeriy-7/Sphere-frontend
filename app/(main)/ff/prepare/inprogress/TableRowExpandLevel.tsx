    ...
    <TableRowExpand isTransparent colSpan={row.getVisibleCells().length}>
      {row.original.subRows?.map((subRow, index) => (
        <div
          key={subRow.uuid}
          className={
            "bg-white border dark:bg-transparent rounded-lg p-2 text-min text-center flex flex-wrap gap-4 [&+&]:mt-5"
          }
        >
          <MarketImg />
          <DPItem>
          ...
          <DPItem>
            ...
            <DPBody className={"text-xs font-medium text-primary"}>
              1 200
            </DPBody>
            <DPTitle>План (ед)</DPTitle>
            <DPBody className={"text-xs font-medium text-primary"}>
                        ...
                        >
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              newValue[itemIndex] = Number.parseInt(
                                e.target.value,
                              );
