...
import { fuzzyFilter, getColSizeList } from '@/lib/TableHelpers';
...
export default function PrepareCompletedPage() {
  // Fetch WB cabinet information
  const { wbCabinetInfo } = useWbCabinetInfo();

  // Fetch deliveries with COMPLETED status
  ...
  const data = useMemo((): DataRow[] => {
    ...
    return deliveriesData.items.map((delivery) => ({
      ...
      // Additional fields for completed deliveries
  ...
  if (!data.length) {
    return (
      <div className="py-8 text-center">
        ...
        <p className="text-gray-500 mt-2">В настоящее время нет выполненных задач</p>
      </div>
    );
  }
